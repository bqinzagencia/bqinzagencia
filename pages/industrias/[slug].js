...
export default function IndustriaDetalle({ industria }) {
  ...
  return (
    <>
      <Head>
        <title>{industria.nombre} — BQinzagencIA · Cero citas perdidas</title>
        ...
        <meta property="og:title" content={`${industria.nombre} — BQinzagencIA · Cero citas perdidas`} />
        <meta property="og:description" content="Cero citas perdidas. Sin tocar el teléfono. Nuestro agente de IA atiende cada llamada y mensaje de WhatsApp mientras tú tienes las manos en el tratamiento." />
        <meta property="og:image" content={industria.img || '/logo.png'} />
        <meta property="og:type" content="website" />
        <meta property="og:locale" content="es_ES" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${industria.nombre} — BQinzagencIA`} />
        <meta name="twitter:description" content="Cero citas perdidas. Agente IA para centros de estética y belleza en España." />
      ...
      <section style={{ minHeight: '70vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '120px 60px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Imagen real en lugar del emoji grande */}
        <div style={{
          width: 88, height: 88, borderRadius: 22, overflow: 'hidden',
          marginBottom: 24, border: `2px solid ${colorBorder}`,
          boxShadow: `0 0 32px ${colorBg}`,
          flexShrink: 0,
        }}>
          {industria.img
            ? <img src={industria.img} alt={industria.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      ...
      <section id="flujo" style={{ padding: '100px 60px' }}>
        ...
        {/* Imagen real pequeña en el header del chat mockup */}
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {Object.values(INDUSTRIAS_DETALLE).filter(ind => ind.slug !== industria.slug).map(ind => (
            <Link key={ind.slug} href={`/industrias/${ind.slug}`}
              style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#111318', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 100, padding: '8px 16px', fontSize: 14, color: '#9CA3AF', textDecoration: 'none', transition: 'all 0.2s' }}
              ...
              {ind.img
                ? <img src={ind.img} alt={ind.nombre} style={{ width: 22, height: 22, borderRadius: 6, objectFit: 'cover', flexShrink: 0 }} />
                : <span>{ind.emoji}</span>
              }
              {ind.nombre}
            </Link>
      ...
      <section style={{ padding: '100px 60px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Imagen real en el CTA final */}
        <div style={{ width: 72, height: 72, borderRadius: 20, overflow: 'hidden', margin: '0 auto 20px', border: `2px solid ${colorBorder}`, position: 'relative' }}>
          {industria.img
            ? <img src={industria.img} alt={industria.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            : <div style={{ width: '100%', height: '100%', background: colorBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>{industria.emoji}</div>
          }
